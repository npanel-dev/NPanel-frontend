...
#######################################################
# NPanel 一键安装脚本
# 支持: Docker Compose 源码构建部署
# 用法: curl -fsSL https://npanel.dev/scripts/zh/install-npanel.sh | bash
#######################################################
...
check_requirements() {
    if ! command -v docker &> /dev/null; then
        ...
        log_info "请先安装 Docker: curl -fsSL https://npanel.dev/scripts/zh/install-docker.sh | sudo bash"
        exit 1
    fi
    ...
    if ! command -v git &> /dev/null; then
        log_error "Git 未安装"
        log_info "请先安装 git，例如: sudo apt-get install -y git"
        exit 1
    fi
...
fetch_backend() {
    ...
    if [[ -d "$INSTALL_DIR/.git" ]]; then
        ...
        log_error "安装目录非空且不是 git 仓库: $INSTALL_DIR"
        log_info "请换一个路径，例如: INSTALL_DIR=/opt/npanel bash install-npanel.sh"
        exit 1
    else
...
show_access_info() {
    ...
    echo ""
    echo "========================================"
    log_info "NPanel 安装完成"
    echo "========================================"
    echo "本机地址: http://localhost:$HOST_PORT"
    if [[ -n "$public_ip" ]]; then
        echo "公网地址: http://$public_ip:$HOST_PORT"
    fi
    echo ""
    log_info "管理员账号:"
    echo "邮箱: $ADMIN_EMAIL"
    ...
    log_info "查看日志: cd $INSTALL_DIR && docker compose logs -f npanel"
}
